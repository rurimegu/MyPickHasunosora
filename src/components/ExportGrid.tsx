...
const getUnitLogoPath = (unit: Unit): string | null => {
  switch (unit) {
    case Unit.CeriseBouquet:
      return "/logos/cerisebouquet.png";
    case Unit.Dollchestra:
      return "/logos/dollchestra.png";
    case Unit.MiraCraPark:
      return "/logos/miracrapark.png";
    case Unit.EdelNote:
      return "/logos/edelnote.png";
    case Unit.Hasunosora:
      return "/logos/hasunosora.png";
    default:
      return null;
  }
};
...
const getSolidBgStyle = (unit: Unit): string => {
  switch (unit) {
    case Unit.Hasunosora:
      return "linear-gradient(to right, #FEF1DF, #FCFBF9)";
    case Unit.CeriseBouquet:
      return "linear-gradient(to right, #FCF4F7, #FCFBF9)";
    case Unit.Dollchestra:
      return "linear-gradient(to right, #F0FDFC, #FCFBF9)";
    case Unit.MiraCraPark:
      return "linear-gradient(to right, #FDF5D5, #FCFBF9)";
    case Unit.EdelNote:
      return "linear-gradient(to right, #F4F6F8, #FCFBF9)";
    case Unit.Other:
      return "linear-gradient(to right, #F5EDFC, #FCFBF9)";
    default:
      return "linear-gradient(to right, #FCFBF9, #FCFBF9)";
  }
};
      ...
      >
        ...
        >
          <div className="flex flex-col">
            <span className="font-serif text-[18px] font-bold text-slate-800 tracking-wider">
              SELECTIONS
            </span>
          </div>
        </div>
        ...
        >
          {cols.map((col) => (
            <div key={col.id} className="py-2 border-b-2 border-black/5">
              <div className="font-serif text-[24px] font-bold text-slate-900 tracking-wide">
                {col.name}
              </div>
              <div className="text-[13px] text-slate-400 tracking-wider font-semibold uppercase mt-1">
                {col.label}
              </div>
            </div>
          ))}
            ...
            >
              ...
              >
                {getUnitLogoPath(row.id) ? (
                  ...
                  <div className="flex flex-col items-center text-center">
                    <span className="font-serif text-[20px] font-bold text-slate-900 tracking-wide">
                      {row.name}
                    </span>
                    <span className="text-[14px] text-slate-500 font-normal mt-1">
                      {row.nameJa}
                    </span>
                  </div>
                )}
              ...
              >
                {cols.map((col) => {
                  ...
                  const cellSongs = SONGS.filter(
                    (s) => s.unit === row.id && s.class === col.id,
                  );
                  ...
                  if (isDisabled) {
                    return (
                      ...
                      >
                        ...
                        <span className="text-[13px] tracking-widest font-bold uppercase text-center mt-2.5">
                          Empty
                        </span>
                      </div>
                    );
                    ...
                    >
                      {song ? (
                        <div className="w-full h-full absolute inset-0">
                          ...
                          {showTitles && (
                            <div
                              ...
                              style={{
                                background:
                                  "linear-gradient(to top, rgba(0, 0, 0, 0.85) 0%, rgba(0, 0, 0, 0.4) 20%, rgba(0, 0, 0, 0) 45%)",
                              }}
                            >
                              <span className="text-white font-sans font-black text-[18px] tracking-wide text-center drop-shadow-lg leading-normal">
                                {song.title.ja}
                              </span>
                            </div>
                          )}
                        ...
                        <div className="flex flex-col items-center gap-2 text-slate-300 p-4">
                          ...
                          <span className="text-[13px] tracking-widest font-semibold uppercase text-center mt-2.5">
                            No Pick
                          </span>
                        </div>
                      )}
